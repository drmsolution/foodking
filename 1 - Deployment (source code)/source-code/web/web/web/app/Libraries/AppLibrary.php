...
class AppLibrary
{
    ...
    public static function currencyAmountFormat($amount): string
    {
        if (config('app.currency_position') == CurrencyPosition::LEFT) {
            return config('app.currency_symbol') . number_format($amount, config('app.currency_decimal_point'), '.', '');
        }
        return number_format($amount, config('app.currency_decimal_point'), '.', '') . config('app.currency_symbol');
    }
        ...
        if ($dateTime) {
            ...
            if ($startTimeObj && $endTimeObj) {
                ...
                if ($currentTime >= $thirtyMinutesBefore && $currentTime <= $endTimeObj) {
                    ...
                    if (!$pattern) {
                        $pattern = config('app.time_format', 'h:i A');
                    }
                    $explode = explode('-', $dateTime);
                    if (count($explode) == 2) {
                        return Carbon::parse(trim($explode[0]))->format($pattern) . ' - ' . Carbon::parse(trim($explode[1]))->format($pattern);
