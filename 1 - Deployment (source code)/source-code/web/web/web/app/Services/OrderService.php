    ...
    public function myOrderStore(OrderRequest $request): object
    {
        try {
            DB::transaction(function () use ($request) {
                ...
                $processed = $this->processOrderItems($this->order, json_decode($request->items));

                $this->setOrderSerial($this->order);
                $this->order->total_tax = $processed['totalTax'];
                $this->order->save();

                $this->saveOrderAddress($this->order, $request->address_id, Auth::user()->id);

                $this->saveOrderCoupon($this->order, $request->coupon_id, Auth::user()->id, $request->discount);
    ...
    public function posOrderStore(PosOrderRequest $request): object
    {
        try {
            DB::transaction(function () use ($request) {
                ...
                $this->order->total_tax = $processed['totalTax'];
                $this->calculateDeliveryTime($this->order);
                $this->order->save();

                $this->saveOrderAddress($this->order, $request->address_id, $request->customer_id);
            });
            return $this->order;
        } catch (Exception $exception) {
            DB::rollBack();
                    ...
                    if ($request->status == OrderStatus::REJECTED || $request->status == OrderStatus::CANCELED) {
                        if ($order->transaction) {
                            app(PaymentService::class)->cashBack(
                                $order,
                                'credit',
                                intval(round(microtime(true) * 1000)) . rand(1000, 9999)
                            );
                        }
                    }
